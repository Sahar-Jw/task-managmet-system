  ...
  useEffect(() => {
    ...
    const taskRequest = isAdmin
      ...
      : Promise.all([
          ...
          TasksApi.assignedByMe({ limit: '100' }),
        ]).then(([assignedToMe, createdByMe]) => {
          const byId = new Map(
            [...assignedToMe.items, ...createdByMe.items].map((item) => [item.id, item]),
          );
